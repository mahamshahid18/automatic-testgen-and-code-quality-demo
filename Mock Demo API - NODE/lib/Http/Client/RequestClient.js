...
const retryrequest = require('requestretry');
const APIHelper = require('../../APIHelper');

// convert to request's version of http request
const convertHttpRequest = function convertHttpRequest(req) {
    const NetworkErrorOnGET = function NetworkErrorOnGET(err) {
        return err &&
            retryrequest.RetryStrategies.NetworkError.RETRIABLE_ERRORS.indexOf(err.code) > -1 &&
            req.method === 'GET';
    };
    retryrequest.RetryStrategies.NetworkError.RETRIABLE_ERRORS.push('ENOENT');

    const options = {
        url: req.queryUrl,
        method: req.method,
        headers: req.headers,
        retryStrategy: NetworkErrorOnGET,
        maxAttempts: 1,
        retryDelay: 1000,
        timeout: 1000,
    };
...
const executeRequest = function executeRequest(req, callback) {
    ...
    let rq;
    if (req.formData) {
        rq = request(convertedRequest, internalCallback);
    } else {
        rq = retryrequest(convertedRequest, internalCallback);
    }
